...
router.post('/deleteUser/:id', authorize(), deleteById);
router.put('/updateUser/:id', authorize(), updateById);
router.get('/', authorize(Role.Admin), getAll);
router.get('/:id', authorize(), getById);
router.get('/:id/get-refresh-tokens', authorize(), getRefreshTokens);
...
function updateById(req, res, next) {
    // regular users can get their own record and admins can update any record
...
function deleteById(req, res, next) {
    // regular users can get their own record and admins can delete any record
    if (req.params.id !== req.user.id && req.user.role !== Role.Admin) {
        return res.status(401).json({ message: 'Unauthorized' });
    }
